 ...
 * Can be used outside of the hook context
 */
export const isPreferenceEnabled = (
...
export const useDisplayPreferences = () => {
  ...
  const loadDisplayPreferences = useCallback(async () => {
    ...
    try {
      ...
      if (savedPreferences && Array.isArray(savedPreferences)) {
        logger.log(
          'Loaded display preferences:',
          savedPreferences.map((p: DisplayPreference) => ({
            id: p.id,
            enabled: p.enabled,
          })),
        );
  ...
  const saveDisplayPreferences = useCallback(
    async (preferences: DisplayPreference[]) => {
      try {
